(ns kaspersky-ti.main
  "Kotodama WASM entrypoint for the Kaspersky_ti clean-room actor (L4) — Clojure port.
  ...
  facts (`kaspersky_ti.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "kaspersky_ti")
(def tier "L4")
...
(def entity-specs
  [{:entity "Indicator"      :plural "indicators"      :id-prefix "kaspersk_ind"
    :fields [:type :value :confidence :firstSeen]      :required [:type :value]
    :coerce {:confidence :int}                :refs {}}
   {:entity "Vulnerability"  :plural "vulnerabilities" :id-prefix "kaspersk_vul"
    :fields [:cveId :cvss :description :published]     :required [:cvss :description]
    :coerce {:cvss :float}                  :refs {}}
   {:entity "ThreatActor"    :plural "threatactors"    :id-prefix "kaspersk_thr"
    :fields [:name :motivation :sophistication]        :required [:name :motivation]
    :coerce {}                              :refs {}}
   {:entity "Campaign"       :plural "campaigns"       :id-prefix "kaspersk_cam"
    :fields [:name :actorId :firstSeen]                :required [:name :firstSeen]
    :coerce {}                              :refs {:actorId "ThreatActor"}}
   {:entity "Host"           :plural "hosts"           :id-prefix "kaspersk_hos"
    :fields [:ipAddress :asn :country :openPorts]     :required [:ipAddress :asn]
    :coerce {}                              :refs {}}
   {:entity "Technique"      :plural "techniques"      :id-prefix "kaspersk_tec"
...
(defn emit-facts
  "EAVT facts for one record: {\"kaspersky_ti.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "kaspersky_ti-compat" :tier tier :entities entities} 200])
...
(defn start! [] :kaspersky_ti-compat/ready)
