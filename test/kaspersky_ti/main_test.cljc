(ns kaspersky-ti.main-test
  "Contract + behavioral test for the kaspersky_ti-compat L4 actor (cljc port).
...
(deftest healthz
  (let [[body status] (m/healthz)]
    (is (= 200 status)) (is (= "kaspersky_ti-compat" (:actor body))) (is (= (set m/entities) (set (:entities body))))))

#?(:clj (defn -main [& _]
          (let [{:keys [fail error]} (run-tests 'kaspersky-ti.main-test)]
